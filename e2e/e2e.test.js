import puppetteer from 'puppeteer';
import { fork } from 'child_process';

jest.setTimeout(30000); // default puppeteer timeout

describe('Credit Card Validator form', () => {
  let browser = null;
  let page = null;
  let server = null;
  const baseUrl = 'http://localhost:9000';

  beforeAll(async () => {
    server = fork(`${__dirname}/e2e.server.js`);
    await new Promise((resolve, reject) => {
      server.on('error', reject);
      server.on('message', (message) => {
        if (message === 'ok') {
          resolve();
        }
      });
    });

    browser = await puppetteer.launch({
      // headless: false, // show gui
      // slowMo: 250,
      // devtools: true, // show devTools
    });
    page = await browser.newPage();
  });

  afterAll(async () => {
    await browser.close();
    server.kill();
  });

  test('check valid', async () => {
    await page.goto(baseUrl);
    const form = await page.$('[data-widget=card-form-widget]');
    const input = await form.$('[data-id=card-input]');
    await input.type('4103186397422796');
    const submit = await form.$('[data-id=card-submit]');
    submit.click();
    await page.waitForSelector('[data-id=card-input].valid');
  });

  test('check invalid', async () => {
    await page.goto(baseUrl);
    const form = await page.$('[data-widget=card-form-widget]');
    const input = await form.$('[data-id=card-input]');
    await input.type('410318639742296');
    const submit = await form.$('[data-id=card-submit]');
    submit.click();
    await page.waitForSelector('[data-id=card-input].invalid');
  });
});
